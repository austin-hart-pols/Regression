...
- Slope coefficient $b_1$ represents expected change in the mean of $Y$ for every one-unit increase in $X$.

- Intercept $a$ is the mean of $Y$ when $X$ equals zero.
...
- Calculate $t$
  - $t=\frac{b_1}{se(b_1)}$
  - For 5% significance, reject $H_0$ if $t>1.96$
...
```{r, results ='asis'}
# Table of results
  stargazer(e1, type = 'html', style = 'apsr', title = "OLS estimates", keep.stat = "n")
```
]
...
.pull-right[
  ...
  Across DC public schools, a one-point increase in student math proficiency is associated with a 0.94-point increase in average language proficiency.  
...
> Estimate = Truth + Error + Bias

- A confounder, $Z$, is a common cause of $X$ and $Y$

- Confounders induce bias
...
- Gives 0 weight to any third var, Z
...
- Then $\beta_1^* \neq \beta_1$ if X and Z are correlated

- Specifically, $\beta_1^*$ includes the association b/w X and Y *AND* some of the association between Z and Y.
...
- Change in mean $Y$ for unit-increase in $X$
- ...independent of Z
...
- Use graph, summary stats, and text to describe `party.id`


---
# EC Quiz

> Evaluate relationship between attitudes about feminists (Y) and racial resentment (X) using OLS regression. Present and interpret a single table with unconditional and conditional estimates.
>
> Submit your table and interpretation in a document (.doc or .pdf) on Canvas.
